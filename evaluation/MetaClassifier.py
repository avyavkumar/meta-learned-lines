...
from datautils.LEOPARDEncoderUtils import read_test_data, get_model, get_tokenizer
...
class MetaClassifier:
    ...
    def trainPrototypes(self, params, training_params, training_set):
        ...
        sentences = training_set['sentences']
        ...
        for test_i in range(len(test_validation_sentences)):
            inputs = self.tokenizer(test_validation_sentences[test_i], return_tensors="pt").to(DEVICE)
        ...
        for idx, line in enumerate(self.lines):
            ...
            filteredTrainingData, filteredTrainingLabels_1 = self.filterEncodingsByLabels(line.getLabels(), encodings,
                                                                                          training_labels)
            filteredTrainingSentences, filteredTrainingLabels_2 = self.filterSentencesByLabels(line.getLabels(),
                                                                                               sentences,
                                                                                               training_labels)
            ...
            filteredTestValidationData, filteredTestValidationLabels_1 = self.filterEncodingsByLabels(line.getLabels(),
                                                                                                      test_validation_encodings,
                                                                                                      test_validation_labels)
            filteredTestValidationSentences, filteredTestValidationLabels_2 = self.filterSentencesByLabels(
                line.getLabels(),
                test_validation_sentences,
                test_validation_labels)
            ...
            training_dataset = SentenceEncodingDataset(filteredTrainingSentences, filteredTrainingData,
                                                       filteredTrainingLabels)
            train_loader = torch.utils.data.DataLoader(training_dataset, **training_params)
            test_validation_dataset = SentenceEncodingDataset(filteredTestValidationSentences,
                                                              filteredTestValidationData, filteredTestValidationLabels)
            ...
            for epoch in range(epochs):  # loop over the dataset multiple times
                ...
                for i, data in enumerate(train_loader, 0):
                    ...
                    outputs, distances_1, distances_2 = self.computeLabelsAndDistances(sentences, encodings,
                    ...
                    with torch.no_grad():
                        ...
                        for j, val_data in enumerate(test_validation_loader, 0):
                            ...
                            val_outputs, _, _ = self.computeLabelsAndDistances(val_sentences, val_encodings,
                                                                               prototype_1, prototype_2)
                            val_losses = criterion(val_outputs, val_labels.to(DEVICE))
                            val_accuracies.append(
                                accuracy_score(val_labels, torch.argmax(val_outputs, dim=1).detach().cpu().numpy()))
    ...
    def getOptimiser(self, model, params):
        return AdamW([{'params': model.metaLearner.parameters()},
                      {'params': model.linear.parameters(), 'lr': params['learning_rate'][params['shot']]}],
                     lr=params['learning_rate'][params['shot']])
    ...
    def computeLabelsAndDistances(self, sentences, encodings, prototype_1, prototype_2):
        ...
        for i in range(encodings.shape[0]):
            distances_1.append(
                np.linalg.norm(encodings[i].detach().cpu().numpy() - prototype_1.getLocation().detach().cpu().numpy()))
            distances_2.append(
                np.linalg.norm(encodings[i].detach().cpu().numpy() - prototype_2.getLocation().detach().cpu().numpy()))
