...
class Reptile(L.LightningModule):
    ...
    def configure_optimizers(self):
        optimiser_2 = AdamW(self.metaLearner.parameters(), lr=self.hparams.outerLR, amsgrad=True)
        scheduler_2 = CosineAnnealingLR(optimiser_2,  T_max=self.MAX_STEPS, eta_min=2e-5)
    ...
    def getInnerLoopOptimiser(self, model, classes, train=False):
        return SGD([{'params': model.metaLearner.parameters(), 'lr': self.hparams.innerLR},
                    {'params': model.linear_1.parameters(), 'lr': self.hparams.outputLR},
                    {'params': model.linear_2.parameters(), 'lr': self.hparams.outputLR}], momentum=0.9, nesterov=True)
    ...
    def trainInnerLoop(self, line, supportSet, supportLabels, train=True):
        ...
        innerModel = PrototypeMetaLinearUnifiedModel(metaLearnerClone, classes=len(set(supportLabels)))
            ...
            for metaParam, localParam in zip(self.metaLearner.parameters(), innerModel.metaLearner.parameters()):
                if metaParam.requires_grad:
                    if metaParam.grad is None:
                        metaParam.grad = torch.zeros(localParam.shape).to(DEVICE)
                    metaParam.grad += metaParam - localParam
        else:
    ...
    def runTrainingWorkflow(self, batch):
        for episode_i in range(len(batch[0])):
            ...
            self.runOuterLoop(supportLines, None, None, innerModel, None, None, train=True)
