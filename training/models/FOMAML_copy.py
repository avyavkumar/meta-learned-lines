...
class FOMAML(L.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        return {
            "optimizer": optimiser,
            "lr_scheduler": {
                "scheduler": MultiStepLR(optimiser, milestones=[125], gamma=0.1)
            }
        }
    ...
    def runInnerLoopTrainingStep(self, line, model_1, model_2, optimisers, filteredTrainingSentences, filteredTrainingEncodings,
        ...
        print("inner loop training loss is", round(sum(training_losses), 3), "and accuracy is", round(accuracy_score(correctLabels, predictions), 3))
    ...
    def trainInnerLoop(self, line: Line, supportSet, supportEncodings, supportLabels, train=True):
        ...
        innerLoopModel_1.to(DEVICE)
        innerLoopModel_2.to(DEVICE)
        # get optimisers
    ...
    def runOuterLoop(self, supportLines, querySentences, queryLabels, train=True):
        ...
        for i in range(len(supportLines)):
            ...
            if requiredQueryEncodings.shape[0] > 0:
                requiredQueryLabels = [int(queryLabels[x]) for x in range((len(assignments))) if assignments[x] == i]
                trainingParams = {
                    'batch_size': 64
                }
                ...
                for j, data in enumerate(trainLoader, 0):
                    if train:
                        ...
                        outerLoopLoss += losses_j.mean().item()
                        predictions_i = torch.argmax(outputs, dim=1).tolist()
                        ...
                        with torch.no_grad():
                            ...
                            self.actualLabels.extend(labels)
                            self.losses.extend(losses_j)
        del queryEncodings, requiredQueryEncodings
        if train:
            ...
            print("outer loop episodic validation accuracy is", round(accuracy_score(outerLoopLabels, outerLoopPredictions), 3), "and loss is", round(outerLoopLoss, 3))
            ...
            self.val_episode += 1
            self.val_episode %= 3 # since we have 8 episodes in a validation set
    ...
    def runMetaWorkflow(self, batch, train=True):
        for episode_i in range(len(batch[0])):
            ...
            del supportLines, supportEncodings
        if train:
            print("aggregated outer loop accuracy is", round(accuracy_score(self.actualLabels, self.predictions), 3), "and total loss is", round(sum(self.losses).item(), 3))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        print("validation accuracy for the validation set is", round(accuracy_score(self.actualLabels, self.predictions), 3), "and the loss is", round(sum(self.losses).item(), 3), "\n")
